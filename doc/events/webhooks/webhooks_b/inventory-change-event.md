...
```ruby
# Implementation example of handling the `inventoryChangeEvent` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InventoryStockIncreaseEvent) || event.is_a?(InventoryStockDecreaseEvent) || event.is_a?(InventoryStockDepletedEvent)
