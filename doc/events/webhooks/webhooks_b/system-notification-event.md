...
```ruby
# Implementation example of handling the `systemNotificationEvent` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SystemAlertNotificationEvent) || event.is_a?(SystemMaintenanceNotificationEvent) || event.is_a?(SystemPerformanceNotificationEvent)
