...
This event's request payload is of type [Integer].
...
```ruby
# Implementation example of handling the `intEvent` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    case event
    when Integer
      puts 'IntEvent received'
      # TODO: Add intevent handling
