...
This event's request payload is of type [Hash[String, String]].
...
```ruby
# Implementation example of handling the `stringMapEvent` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(Hash) && event.all? { |k,v| k.is_a?(String) && v.is_a?(String) }
      puts 'StringMapEvent received'
      # TODO: Add stringmapevent handling
