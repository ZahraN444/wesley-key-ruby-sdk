...
This event's request payload is of type [Array[Float]].
...
```ruby
# Implementation example of handling the `numberListEvent` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(Array) && event.all? { |x| x.is_a?(Float) }
      puts 'NumberListEvent received'
      # TODO: Add numberlistevent handling
