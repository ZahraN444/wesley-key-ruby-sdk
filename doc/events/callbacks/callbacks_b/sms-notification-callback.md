...
```ruby
# Implementation example of handling the `smsNotificationCallback` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(NotificationCallback) && event.notification_type == 'sms'
